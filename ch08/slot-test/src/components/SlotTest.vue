<template>
  <div>
    ...
    <CheckBox2 v-for="item in items" :key="item.id" :id="item.id" :checked="item.checked"
      @check-changed="CheckBoxChanged">

      <span v-if="item.checked" style="color:blue; text-decoration:underline;">
        <i>{{ item.label }} - checked</i></span>
      <span v-else style="color:gray">{{ item.label }} - unchecked</span>
